...
const Orders = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                // Ensure we always set an array, even if the API returns something weird
    ...
    return (
        <div className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 sm:px-6 lg:px-8 lg:py-12 selection:bg-accent/30 font-sans text-slate-900">
            ...
            {loading ? (
                ...
                <div className="space-y-6">
                    {invoices.map((inv) => {
                        ...
                        // Safe fallbacks for data that might be missing
                        const safeAmount = inv.totalAmount || inv.amount || 0;
                        const safePaymentTerms = inv.paymentTerms || 'N/A';
                        const safeOrderIdDisplay = inv.orderId?.orderId || inv.orderId || 'N/A';
                        const safeTrackingId = inv.orderId?._id || inv.orderId || inv._id;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex w-full flex-col items-stretch gap-3 sm:flex-row md:w-auto md:flex-col md:items-end">
                                    ...
                                    {isOrder && (
                                        <Link
                                            to={`/orders/${safeTrackingId}/track`}
