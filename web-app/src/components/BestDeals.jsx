                ...
                {isLoading ? (
                    ...
                    <div className="deals-grid">
                        {deals.map((deal) => {
                            ...
                            return (
                                <div className="deal-card" key={deal._id} id={`deal-${deal._id}`} style={{ position: 'relative' }}>
                                    {badge && <div className="deal-badge">{badge}</div>}
                                    <button
                                        className={`pc-wishlist-btn ${isInWishlist(deal._id) ? 'pc-wishlist-active' : ''}`}
                                        style={{ position: 'absolute', top: '12px', right: '12px', zIndex: 10, background: 'white', border: 'none', borderRadius: '50%', padding: '6px', cursor: 'pointer', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}
                                        onClick={(e) => {
                                            e.preventDefault();
                                    ...
                                    >
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill={isInWishlist(deal._id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    ...
                                    <div className="deal-info">
                                        ...
                                        {cartItem ? (
                                            <div className="cart-quantity-controls">
                                                <button onClick={(e) => { e.preventDefault(); updateQuantity(deal._id, -1); }}>-</button>
                                                <span>{cartItem.quantity}</span>
                                                <button onClick={(e) => { e.preventDefault(); updateQuantity(deal._id, 1); }}>+</button>
                                            </div>
                                        ) : (
                                            <button className="btn-add-cart" id={`btn-add-cart-${deal._id}`} onClick={(e) => {
                                                e.preventDefault();
                                                addToCart({
                                                    id: deal._id,
                                                    name: deal.title,
