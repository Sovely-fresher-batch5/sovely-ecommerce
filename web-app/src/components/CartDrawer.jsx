...
function CartDrawer({ isOpen, onClose }) {
    ...
    const totals = cartItems.reduce(
        (acc, item) => {
            ...
            const gstRate = Number(item.product?.gstPercent) || Number(item.product?.gst) || 18;
            const gstAmount = itemTotal * (gstRate / 100);

            return {
                subtotal: acc.subtotal + itemTotal,
                gst: acc.gst + gstAmount,
                weight: acc.weight + (Number(item.product?.weight) || 0.5) * quantity,
            };
        },
                    ...
                    <div>
                        <h2 className="flex items-center gap-2 text-xl font-extrabold text-slate-900">
                            ...
                            <span className="bg-primary rounded-full px-2.5 py-1 text-xs font-bold text-white shadow-sm">
                                {cartItems.reduce((acc, i) => acc + (Number(i.quantity) || 1), 0)} Units
                            </span>
                        </h2>
                                            ...
                                            <div className="flex items-start justify-between gap-2 pr-6">
                                                <div>
                                                    <span className="text-[9px] font-bold tracking-widest text-slate-400 uppercase">
                                                        {product.category || product.productType || 'Item'}
                                                    </span>
                                                    ...
                                                    <div className="flex h-8 items-center rounded-lg border border-slate-200 bg-slate-50 p-0.5">
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => {
                                                                ...
                                                                if (!isNaN(val)) setExactQuantity(itemKey, val);
                                                            }}
                                                            onBlur={(e) => {
                                                                ...
                                                                if (isNaN(val) || val < moq) val = moq;
                                                                    ...
                                                                    if (remainder !== 0) val = val - remainder;
                                                                }
                    ...
                    <div className="relative z-10 border-t border-slate-200 bg-white p-6 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
                        ...
                        <div className="mb-6 space-y-2">
                            ...
                            <div className="flex items-center justify-between text-sm text-slate-500">
                                ...
                                <span className="font-semibold text-slate-700">
                                    + ₹{totals.gst.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                                </span>
                            </div>
                            <div className="mt-2 flex items-end justify-between border-t border-slate-100 pt-3">
                                ...
                                <span className="text-2xl font-extrabold tracking-tight text-slate-900">
                                    ₹{(totals.subtotal + totals.gst).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                                </span>
                            </div>
                        </div>
